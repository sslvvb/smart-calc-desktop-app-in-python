"""
Module for initializing the user interface components.
"""

import customtkinter as ctk

from .utils import get_code_main_color, get_code_bg_color, validate_float_input
...
class UiInitializer:
    ...
    def update_config(self, key: str, value: str):
        if key == "main_color":
            for btn in self.view.buttons:
                if btn.cget("fg_color") == get_code_main_color(
                        self.view.main_color):
            ...
            self.view.history_menu.configure(
                fg_color=get_code_main_color(value),
    ...
    def _init_radio_buttons(self, key, values, row):
        ...
        for idx, value in enumerate(values, start=1):
            radio = ctk.CTkRadioButton(
                self.view,
                text=value,
                command=lambda k=key, v=value: self.view.config_event_callback(
                    k, v),
                variable=var,
                value=value,
                border_color="#343230",
    ...
    def _init_entry(self):
        expression_entry = ctk.CTkEntry(
            self.view, textvariable=self.view.expression_var
        )
        ...
        for var, initial_value, grid_params in entry_settings:
            ...
            entry.grid(
                row=grid_params[0], column=grid_params[1],
                columnspan=grid_params[2]
            )
    ...
    def _init_label(self):
        ...
        for label_text, row, column in labels_info:
            label = ctk.CTkLabel(self.view, text=label_text,
                                 text_color="#343230")
